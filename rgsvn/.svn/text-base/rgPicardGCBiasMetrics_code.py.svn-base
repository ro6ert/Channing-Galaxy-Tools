...
def exec_after_process(app, inp_data, out_data, param_dict, tool, stdout, stderr):
    ...
    job_name = param_dict.get( 'out_prefix', 'PicardCGBiasMetrics' )
    title = job_name.translate(trantab)
    html = ['html','%s.html' % title]
    lookup={}
    lookup['html_file'] = html
    info = '%s Picard Collect GC Bias Metrics report created by rgPicardGCBiasMetrics at %s' % (title,timenow())
