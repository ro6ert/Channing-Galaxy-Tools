#!/usr/bin/env python
bwaexe = 'bwargpatch' # normally bwa but we need a patched svn (r47) for read groups
"""
...
def __main__():
    ...
    if options.rgID > '':
        read_group_cmds = '-i %s -m %s %s' % (options.rgID, options.rgSM, read_group_cmds)    # set up aligning and generate aligning command options
    ...
    if options.genAlignType == 'paired':
        ...
        cmd3 = '%s sampe %s %s %s %s %s %s %s >> %s' % (bwaexe, gen_alignment_cmds, read_group_cmds, ref_file_name, tmp_align_out_name, tmp_align_out2_name, options.fastq, options.rfastq, options.output )
    else:
        cmd3 = '%s samse %s %s %s %s %s >> %s' % ( bwaexe, gen_alignment_cmds, read_group_cmds, ref_file_name, tmp_align_out_name, options.fastq, options.output )
    # perform alignments
    buffsize = 1048576
    try:
