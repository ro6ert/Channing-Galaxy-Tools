...
class sortBAM():
    ...
    def __init__(self,opts=None,cl=[],tidy=True):
        ...
        fd,self.tlogname = tempfile.mkstemp(dir=self.opts.tmp_dir,suffix='rgSortBam.log')
    ...
    def runCleanSam(self,infile=None,isSam=True,outBam=False):
        ...
        if isSam:        
            cl = ['java -Xmx2g -jar',self.opts.jar,'INPUT=%s' % infile,'OUTPUT=%s' % tempclean,'VERBOSITY=DEBUG']
        else: # given bam we have to do some work
            sam = self.bamToSam(infile=infile)
            self.delme.append(sam)
            cl = ['java -Xmx2g -jar',self.opts.jar,'INPUT=%s' % sam,'OUTPUT=%s' % tempclean,'VERBOSITY=DEBUG']
        self.run(cl)
    ...
    def sortme(self):
        ...
        if self.opts.runcalmd == "true": # use the -b flag and remember to redirect back into sortedbam
            sortedbam = self.runCalmd(infile=sortedbam)
        # sortedbam is go
...
if __name__ == '__main__':
   ...
   --newformat "$new_format" --tmp_dir "${__new_file_path__}" -m "$html_file" -d "$html_file.files_path"
#if $runCleanSam:
