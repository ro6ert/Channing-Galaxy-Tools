...
manhattan = function(chrom=NULL,offset=NULL,pvals=NULL, title=NULL, max.y="max", 
   suggestiveline=0, genomewide=T, size.x.labels=9, size.y.labels=10, annotate=F, SNPlist=NULL,grey=0, ndata=0) {
        ...
        if (genomewide) { # use bonferroni since might be only a small region?
            genomewideline = -log10(0.05/ndata) }
...
dnames = names(rawd)
cc = dnames[chromcolumn]
oc = dnames[offsetcolumn] 
sortnames = c(cc,oc)
ndata = nrow(rawd)
keepalln = round(ndata/100) # keep all lowest (a lot if 1M p values)
keepmostn = round(ndata/10) # keep most of top 1/10
print(paste('###',length(rawd[,1]),'values read from',infile,'read - now running plots',sep=' '))
for (pvalscolumn in pvalscolumns) {
if (pvalscolumn > 0) 
     {
     cname = dnames[pvalscolumn]
     od = rawd[do.call(order,rawd[cname]),] # order by this pval
     if (ndata > 10000) { 
        # no need unless a lot of data! 
        od$i = c(1:ndata) # index
        k1 = od$i < keepalln
        k2 = od$i < keepmostn
        od$keep = k1 + k2 # becomes 2,1,0 
        dlist = split(od,od$keep) # produces 3 datasets
        nd = lapply(dlist,nrow)
        d1 = dlist[[1]][sample(nd[[1]],nd[[1]]/10,replace=T),]
        d2 = dlist[[2]][sample(nd[[2]],nd[[2]]/3,replace=T),]
        d3 = dlist[[3]]
        trimmedd = rbind(d1,d2,d3)
        } else { trimmedd = od } # small    
     d = trimmedd[do.call(order,trimmedd[sortnames]),] 
     ...
     myqqplot = qq(d[,pvalscolumn],title=mytitle,ndata=ndata)
     ...
     if ((chromcolumn > 0) & (offsetcolumn > 0)) {
         ...
         mymanplot= manhattan(chrom=d[,chromcolumn],offset=d[,offsetcolumn],pvals=d[,pvalscolumn],title=mytitle,grey=grey,ndata=ndata)
