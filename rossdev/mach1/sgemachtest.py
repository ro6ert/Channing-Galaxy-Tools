...
def sgeWait(params=[],machexe='mach1'):
    ...
    jt.remoteCommand = machexe
    jt.args = params
    jt.joinFiles = True
    
    jobid = s.runJob(jt)
    logging.info('Job has been submitted with id %d' % jobid)

    retval = s.wait(jobid, drmaa.Session.TIMEOUT_WAIT_FOREVER)
    logging.info('Job: %s finished with status %s' % (str(retval.jobId), str(retval.hasExited)))
...
class sgeMwait():
    ...
    and tinkered. Takes a list of command lines, submits them to SGE and waits for the last one to finish
    """
    def __init__(self,machexe='mach1',clist=None,nbulks=1,job_chunk=1):
    ...
    def mRun(self):
        ...
        for clist in self.clists:
            if type(clist) == type([]):
                clist = ['%s %s' % (self.machexe,x) for x in clist] # prepend mach
                batch = '\n'.join(clist)
                
                    
            path = self.machexe
            jt = self.init_job_template(jt=s.createJobTemplate(), path=job_path, args=clist, as_bulk=False)
            jobid = s.runJob(jt)
            alljids.append(jobid)
            sleep(1)
        logging.info('# submitted jobids: %s' % alljids)
        s.synchronize(alljids,
...
class machRun():
    ...
    def __init__(self, basename=None, stage1name=None, snpsource=None, hapsource=None, outname=None,
                ...
                chrom=None,offset=None,length=None,outpath=None,useSGE=False,**kwd):
        """
    ...
    def compute(self):
        ...
        if self.greedy:
            cl = '%s --greedy' % cl
        if self.mask:
            cl = '%s --mask %f' % self.mask
        lcl.append(cl)
        logging.info( '## mach run cl = %s' % '\n'.join(lcl))
        if self.useSGE:
            alog = self.runSGE(lcl)
        else:
            alog = self.run(lcl)
        if alog1 > []:
            alog1 += alog
    ...
    def runSGE(self,cl=[]):
        """make a shell script to run the cl and submit to sge        
        """
        script = []
        alog = []
        cd = None
        ...
        submitme.close()
        if cd:
            x = subprocess.Popen('qsub %s' % submitme,shell=True,cwd=cd)
        else:
            x = subprocess.Popen('qsub %s' % submitme,shell=True)
        retval = x.wait()
        if cd: # undo submit cd
            os.chdir(cd)
        alog.append('# runSGE submitted:')
        alog += script
        alog += '# finished at %s' % timenow()
        return alog
...
def mrun():
    ...
    def runOneChrom(chrom=None,p=None,mask=None):
        ...
        m = machRun(basename=options.basefile, stage1name=stage1outname, outname=stage2outname,
            ...
            machexe='mach1',cd=None,nmax=options.nstage1,chrom=chrom,offset=options.offset,
            length=options.length,hapsource=options.happath,useSGE=options.useSGE,
    ...
    a("--useSGE",dest="useSGE",default=False,action='store_true',help='Submit jobs using SGE')
