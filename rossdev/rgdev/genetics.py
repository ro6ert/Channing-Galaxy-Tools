...
class GeneralFeatureFormat(Interval):

    """
    rg version viewable at ucsc of Gff format
    """
    file_ext = "rggff" # distinguish from gff
    ...
    def set_meta( self, dataset, overwrite = True, **kwd ):
        Interval.set_meta( self, dataset, overwrite = overwrite, **kwd )
        return True
...
class Rgenetics(Html):      
    ...
    def generate_primary_file( self, dataset = None ):
        ...
        for composite_name, composite_file in self.get_composite_files( dataset = dataset ).iteritems():
            ...
            if composite_file.optional:
                opt_text = ' (optional)'
            rval.append( '<li><a href="%s" type="application/binary">%s (%s)</a>%s' % ( fn, fn, composite_file.get('description'), opt_text ) )
    ...
    def regenerate_primary_file(self,dataset):
        ...
        for i,fname in enumerate(flist):
            ...
            rval.append( '<li><a href="%s" mimetype="%s">%s</a>' % ( sfname, mt, sfname) )
        rval.append( '</ul></html>' )
