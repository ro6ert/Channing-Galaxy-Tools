...
def exec_after_process(app, inp_data, out_data, param_dict, tool, stdout, stderr):
    ...
    now need to be moved into files in the history
    ...
    newfiles = [x for x in newfiles if len(x) == 3]
    d = os.getcwd()
    ...
    for alist in newfiles:
        ...
        fpath,bname,filext = alist
        fpath = fpath.strip()
        newflist = glob.glob(os.path.join(fpath,'%s.*' % bname))
        newname = bname.strip()
        info = '%s converted by rgLpedSub at %s' % (bname,timenow())
        newdata = app.model.HistoryDatasetAssociation(extension=filext,dbkey=dbkey,info=info,
            name=newname, create_dataset = True )
        data.change_datatype(filext)
