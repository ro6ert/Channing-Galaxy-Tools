...
def subjectRep(froot='cleantest',outfname="srep",logfname='log.log'):
...
def markerRep(froot='cleantest',outfname="mrep",logfname='log.log' ):
...
if __name__ == "__main__":
    ...
    flogf,alogf = tempfile.mkstemp(suffix='.txt')
    f = open(alogf,'w')
    f.close() # just make a file for subsequent appending
    ...
    ['--mendel',],['--check-sex',]]
    vclbase = [plinke,'--noweb','--out',basename,'--bfile',bfn,'--mind','1.0','--geno','1.0','--maf','0.0']
    runPlink(bfn = bfn,ofn = ofn,logf=alogf,plinktasks=plinktasks,cd=newfpath,
    vclbase=vclbase)
    ...
    plog = pruneLD(plinktasks=plinktasks,cd=newfpath,vclbase = vclbase)
    
    repout = os.path.join(newfpath,basename)
    subjects,subjectTops = subjectRep(froot=repout,outfname=asubjf,
                logfname=alogf) # writes the subject_froot.xls file
    markers,markerTops = markerRep(froot=repout,outfname=amarkf,
                logfname=alogf) # marker_froot.xls
    nbreaks = 100
    ...
    html.append('</table>')
    llog = open(alogf,'r').readlines()
    llog += plog # add lines from pruneld log
